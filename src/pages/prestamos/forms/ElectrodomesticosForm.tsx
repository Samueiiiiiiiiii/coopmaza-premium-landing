  ...
  const onSubmit = async (data: ElectrodomesticoFormValues) => {
    try {
      ...
      try {
        ...
        pdf_url = await uploadToB2Client(base64Pdf, 'application/pdf', `solicitud-electrodomesticos-${data.cedula}.pdf`);
      } catch (pdfErr) {
        console.error('Error generando PDF en solicitud electrodomesticos:', pdfErr);
      }
      ...
      await sendLoanNotification({
        cargo: 'PRÉSTAMO DE ELECTRODOMÉSTICOS',
