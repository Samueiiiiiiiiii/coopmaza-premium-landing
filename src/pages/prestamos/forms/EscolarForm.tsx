...
type EscolarFormValues = z.infer<typeof escolarSchema>;
...
export default function EscolarForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<EscolarFormValues>({
    resolver: zodResolver(escolarSchema),
    defaultValues: {
      nacionalidad: 'Dominicana',
    }
  });

  const onSubmit = async (data: EscolarFormValues) => {
    try {
      ...
      try {
        ...
        pdf_url = await uploadToB2Client(base64Pdf, 'application/pdf', `solicitud-escolar-${data.cedula}.pdf`);
      } catch (pdfErr) {
        console.error('Error generando PDF en solicitud escolar:', pdfErr);
      }
      ...
      <Section title="II. Datos del Préstamo">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1.5 md:col-span-2">
            ...
            <Input placeholder="Ej. Diez mil pesos" {...register('monto_letras')} />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Ej. 10,000" type="number" {...register('monto_prestamo')} />
