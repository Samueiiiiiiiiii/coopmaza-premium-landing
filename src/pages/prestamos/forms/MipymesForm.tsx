...
const mipymesSchema = z.object({
  ...
  tipo_negocio: z.enum(['venta_productos', 'servicios', 'produccion_artesania', 'otro'], { message: 'Requerido' }),
  tipo_negocio_otro: z.string().optional(),
  direccion_negocio: z.string().min(5, 'Requerido'),
  formalizado: z.enum(['si', 'no'], { message: 'Requerido' }),
  tiempo_operacion: z.string().min(1, 'Requerido'),
  ...
  ref_tipo_empresa: z.string().min(2, 'Requerido'),
...
type MipymesFormValues = z.infer<typeof mipymesSchema>;
...
export default function MipymesForm() {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<MipymesFormValues>({
    resolver: zodResolver(mipymesSchema),
    defaultValues: {
      nacionalidad: 'Dominicana',
    }
  });

  const watchTipoNegocio = watch('tipo_negocio');

  const onSubmit = async (data: MipymesFormValues) => {
    try {
      ...
      try {
        const pdfData = {
          ...
          cargo: 'PRESTAMO_MIPYMES',
          dependientes: JSON.stringify({
            nacionalidad: data.nacionalidad,
            numero_cuenta: data.numero_cuenta,
            nombre_negocio: data.nombre_negocio,
            tipo_negocio: data.tipo_negocio,
            tipo_negocio_otro: data.tipo_negocio_otro,
        ...
        pdf_url = await uploadToB2Client(base64Pdf, 'application/pdf', `solicitud-mipymes-${data.cedula}.pdf`);
      } catch (pdfErr) {
        console.error('Error generando PDF en solicitud mipymes:', pdfErr);
      }
    ...
    return (
      <div className="bg-white rounded-2xl p-10 text-center shadow-lg border border-brand-100/50 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 mb-8 text-lg">
          Su solicitud de Préstamo para MIPYMES ha sido enviada exitosamente. Nuestro equipo la revisará y le contactará pronto.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 animate-fade-in-up">
      ...
      {/* Punto II - Datos del Negocio */}
      <Section title="II. Datos del Negocio/Emprendimiento">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1.5 md:col-span-2">
            <Label>Nombre del Negocio/Proyecto <span className="text-red-500">*</span></Label>
            <Input placeholder="Ej. Colmado El Primo" {...register('nombre_negocio')} />
            {errors.nombre_negocio && <p className="text-sm text-red-600">{errors.nombre_negocio.message?.toString()}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Select {...register('tipo_negocio')}>
              <option value="">Seleccione...</option>
              <option value="venta_productos">Venta de productos</option>
              <option value="servicios">Servicios</option>
              <option value="produccion_artesania">Producción/Artesanía</option>
              <option value="otro">Otro</option>
            </Select>
            {errors.tipo_negocio && <p className="text-sm text-red-600">{errors.tipo_negocio.message?.toString()}</p>}
          </div>

          {watchTipoNegocio === 'otro' && (
            <div className="space-y-1.5">
              <Label>Especifique otro tipo <span className="text-red-500">*</span></Label>
              <Input placeholder="Especifique..." {...register('tipo_negocio_otro')} />
            </div>
          )}

          <div className="space-y-1.5 md:col-span-2">
            <Label>Dirección o Ubicación Principal del Negocio <span className="text-red-500">*</span></Label>
            <Input placeholder="Calle, Sector..." {...register('direccion_negocio')} />
            {errors.direccion_negocio && <p className="text-sm text-red-600">{errors.direccion_negocio.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5">
            <Label>¿El negocio está formalizado (RNC)? <span className="text-red-500">*</span></Label>
            <Select {...register('formalizado')}>
              <option value="">Seleccione...</option>
              <option value="si">Sí</option>
              <option value="no">No</option>
            </Select>
            {errors.formalizado && <p className="text-sm text-red-600">{errors.formalizado.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5">
            <Label>Tiempo de Operación del Negocio <span className="text-red-500">*</span></Label>
            <Input placeholder="Ej. 2 años" {...register('tiempo_operacion')} />
            {errors.tiempo_operacion && <p className="text-sm text-red-600">{errors.tiempo_operacion.message?.toString()}</p>}
          </div>
        </div>
      </Section>
      ...
      <Section title="III. Datos del Préstamo">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1.5 md:col-span-2">
            ...
            <Input placeholder="Ej. Doscientos mil pesos" {...register('monto_letras')} />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Ej. 200,000" type="number" {...register('monto_prestamo')} />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Ej. 24 meses" {...register('plazo_prestamo')} />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Ej. 50" type="number" {...register('cantidad_acciones')} />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Ej. Compra de mercancía" {...register('fines')} />
            {errors.fines && <p className="text-sm text-red-600">{errors.fines.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5">
            <Label>Fecha de vencimiento <span className="text-red-500">*</span></Label>
            <Input placeholder="Ej. DD/MM/AAAA" {...register('fecha_vencimiento')} />
            {errors.fecha_vencimiento && <p className="text-sm text-red-600">{errors.fecha_vencimiento.message?.toString()}</p>}
          </div>
        </div>
      </Section>
      ...
      <Section title="V. Referencias Comerciales">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1.5 md:col-span-2">
            ...
            <Input placeholder="Ej. Proveedor XYZ" {...register('ref_nombres_empresa')} />
            {errors.ref_nombres_empresa && <p className="text-sm text-red-600">{errors.ref_nombres_empresa.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5">
            <Label>Dirección <span className="text-red-500">*</span></Label>
            <Input placeholder="Ej. Calle Principal #1" {...register('ref_direccion')} />
            {errors.ref_direccion && <p className="text-sm text-red-600">{errors.ref_direccion.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5">
            <Label>Teléfono <span className="text-red-500">*</span></Label>
            <Input placeholder="(000) 000-0000" type="tel" {...register('ref_telefono')} />
            {errors.ref_telefono && <p className="text-sm text-red-600">{errors.ref_telefono.message?.toString()}</p>}
          </div>

          <div className="space-y-1.5 md:col-span-2">
            <Label>Tipo de empresa <span className="text-red-500">*</span></Label>
            <Input placeholder="Ej. Distribuidora Mayorista" {...register('ref_tipo_empresa')} />
            {errors.ref_tipo_empresa && <p className="text-sm text-red-600">{errors.ref_tipo_empresa.message?.toString()}</p>}
          </div>
        </div>
      </Section>
