...
const gerencialSchema = z.object({
...
type GerencialFormValues = z.infer<typeof gerencialSchema>;
...
export default function GerencialForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<GerencialFormValues>({
    resolver: zodResolver(gerencialSchema),
    defaultValues: {
      nacionalidad: 'Dominicana',
    }
  });

  const onSubmit = async (data: GerencialFormValues) => {
    try {
      ...
      try {
        ...
        const pdfBlob = await pdf(<PrestamoGerencialPDF data={pdfData} />).toBlob();
        const reader = new FileReader();
        const base64Pdf = await new Promise<string>((resolve) => {
          reader.onloadend = () => resolve(reader.result as string);
          reader.readAsDataURL(pdfBlob);
        });
        pdf_url = await uploadToB2Client(base64Pdf, 'application/pdf', `solicitud-gerencial-${data.cedula}.pdf`);
      } catch (pdfErr) {
        console.error('Error generando PDF en solicitud gerencial:', pdfErr);
      }
    ...
    return (
      <div className="bg-white rounded-2xl p-10 text-center shadow-lg border border-brand-100/50 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 mb-8 text-lg">
          Su Solicitud de Préstamo Gerencial ha sido enviada exitosamente. Nuestro equipo la revisará y le contactará pronto.
        </p>
